...
const FIELDS = ["nome_filamento", "material_filamento", "cor_filamento", "custo_medio_brl"];
const NUMERIC = ["custo_medio_brl"];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!requiredValue) return NextResponse.json({ ok: false, error: "O campo nome_filamento é obrigatório." }, { status: 400 });
