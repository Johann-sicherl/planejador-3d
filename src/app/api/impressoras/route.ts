...
const TABLE = "cadastro_impressoras";
const ID_COL = "id_impressora";
const FIELDS = ["nome_impressora", "tempo_operacao_disponivel_dia"];
const NUMERIC = ["tempo_operacao_disponivel_dia"];
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredValue = String(body.nome_impressora ?? "").trim();
    if (!requiredValue) return NextResponse.json({ ok: false, error: "O campo nome_impressora é obrigatório." }, { status: 400 });
