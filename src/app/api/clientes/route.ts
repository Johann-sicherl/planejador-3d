...
const ID_COL = "id_cliente";
const FIELDS = ["nome_cliente", "endereco", "telefone", "cpf", "cnpj"];
const NUMERIC = [""];
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredValue = String(body.nome_cliente ?? "").trim();
    if (!requiredValue) return NextResponse.json({ ok: false, error: "O campo nome_cliente é obrigatório." }, { status: 400 });
