...
export default function Page() {
  ...
  const { data, loading, erro, setErro, reload } = useCrudList<Registro>("/api/estoque");
  const [options, setOptions] = useState<any>(null);
  const [salvando, setSalvando] = useState(false);
  const [mensagem, setMensagem] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [id_filamento, setId_filamento] = useState("");
  const [qtd_estoque_gramas, setQtd_estoque_gramas] = useState("");
...
getJson<any>("/api/options").then((r) => setOptions(r[0] || null)).catch(() => {});
  ...
  function resetForm() {
    setEditingId(null);
    setId_filamento("");
    setQtd_estoque_gramas("");
  }

  function fillForEdit(row: Registro) {
    setEditingId(String(row["id_filamento"] ?? ""));
    setId_filamento(String(row["id_filamento"] ?? ""));
    setQtd_estoque_gramas(String(row["qtd_estoque_gramas"] ?? ""));
  ...
  async function handleDelete(id: string) {
    ...
    try {
      setErro("");
      setMensagem("");
      const response = await fetch("/api/estoque?id=" + id, { method: "DELETE" });
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        id_filamento: id_filamento === "" ? null : Number(id_filamento),
        qtd_estoque_gramas: NumberOrBlank(qtd_estoque_gramas),
      };
  ...
  return (
    <PageShell title="Estoque" description="Controle do estoque em gramas.">
      <div className="grid gap-6 xl:grid-cols-[520px_1fr]">
        <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-black">Filamento</label>
              <select value={id_filamento} onChange={(e) => setId_filamento(e.target.value)} className="w-full rounded-xl border border-zinc-300 px-3 py-2 text-black outline-none focus:border-zinc-500">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-black">Quantidade em gramas</label>
              <input type="number" value={qtd_estoque_gramas} onChange={(e) => setQtd_estoque_gramas(e.target.value)} className="w-full rounded-xl border border-zinc-300 px-3 py-2 text-black outline-none focus:border-zinc-500" placeholder="Quantidade em gramas" />
            </div>
          ...
          <div className="mt-4 overflow-x-auto">
            <LoadingOrEmpty loading={loading} empty={data.length === 0} loadingText="Carregando registros..." emptyText="Nenhum registro cadastrado.">
              <table className="min-w-full border-separate border-spacing-0 text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-zinc-200 px-3 py-3 text-left font-semibold text-black">Filamento</th>
                    <th className="border-b border-zinc-200 px-3 py-3 text-left font-semibold text-black">Quantidade em gramas</th>
                ...
                <tbody>
                  {data.map((row, index) => (
                    <tr key={String(index)}>
                      <td className="border-b border-zinc-100 px-3 py-3 text-black">{row["id_filamento"] ?? ""}</td>
                      <td className="border-b border-zinc-100 px-3 py-3 text-black">{row["qtd_estoque_gramas"] ?? ""}</td>
