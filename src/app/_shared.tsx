      ...
      <div className="flex min-h-screen">
        <aside className="hidden w-72 shrink-0 border-r border-white/10 bg-slate-950/80 p-5 backdrop-blur-xl lg:block">
          <Link href="/dashboard" className="mb-8 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-black text-white">PLANEJADOR 3D</p>
        ...
        <section className="min-w-0 flex-1">
          <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/75 px-5 py-4 backdrop-blur-xl">
            <div className="flex items-center justify-between gap-4">
              <Link
                href="/dashboard"
                className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-slate-300 lg:hidden"
              >
              ...
              <Link
                href="/configuracoes"
                className="rounded-2xl border border-white/10 bg-white/[0.04] p-2 text-slate-300 hover:bg-white/10"
              >
          ...
          <div className="mx-auto max-w-[1800px] space-y-6 px-5 py-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="mb-2 text-xs font-bold uppercase tracking-[0.28em] text-cyan-400">
                  Planejador 3D
                </p>
...
export function GlassCard({
  ...
  return (
    <div
      className={`rounded-3xl border border-white/10 bg-white/[0.045] p-5 shadow-xl shadow-black/20 backdrop-blur-xl ${className}`}
    >
      {children}
    </div>
  );
...
export function Feedback({ erro, mensagem }: { erro: string; mensagem: string }) {
  return (
    <>
      ...
      {mensagem && (
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-medium text-emerald-300">
          {mensagem}
        </div>
      )}
    </>
...
export function ActionButtons({
  ...
  return (
    <div className="flex gap-2">
      <button
        onClick={onEdit}
        className="rounded-xl border border-cyan-400/30 bg-cyan-400/10 px-3 py-1 text-xs font-bold text-cyan-300 hover:bg-cyan-400/20"
      >
        Editar
      </button>
      ...
      <button
        onClick={onDelete}
        className="rounded-xl border border-red-400/30 bg-red-400/10 px-3 py-1 text-xs font-bold text-red-300 hover:bg-red-400/20"
      >
        Excluir
      </button>
    </div>
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      if (
        ...
        !["/login", "/cadastro"].includes(window.location.pathname)
      ) {
